...
own (such as `mattpocock-skills:tdd` and `mattpocock-skills:writing-for-agents`) come
...
Consequence: edit on a branch, and keep the working tree coherent at every commit: leave
no half-written skill or malformed frontmatter in place overnight or between commits.
