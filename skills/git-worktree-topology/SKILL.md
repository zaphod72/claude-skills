...
saved it was luck of style; that style is the second defense: **commands with explicit refs are
...
run: it's the cheapest place to catch the instruction having been ignored, and it confirms itself
...
it's intended, but confirm it's intended; unnoticed it's a silent stall with no error to
surface it.
