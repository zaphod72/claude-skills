...
agent-style (v0.5.0, +RULE-K-M) is 25 rules against recurring failure patterns in LLM-written technical prose: 12 canonical rules from Strunk & White, Orwell, Pinker, and Gopen & Swan, plus 13 patterns observed directly in LLM output, 2022–2026. Run it as one pass over a finished draft, not as 25 separate steps.
...
- **RULE-06 — claim calibration** (high): match the verb to the evidence — "suggests"/"shows" for results, "proves" only for derivations; name the source instead of "prior work shows".
- **RULE-07 — sentence length** (high): split sentences over 30 words; vary length across a paragraph.
- **RULE-08 — avoidable jargon** (medium): "leverage" → "use", "utilize" → "use", "methodology" → "method". Keep jargon that carries distinct technical meaning.
- **RULE-09 — positive form** (medium): "not large" → "small"; don't stage a claim as "X, not Y" unless the rejected Y is specific and informative.
- **RULE-10 — parallel structure** (medium): coordinate items (bullets, "and"/"or" lists) share one grammatical form.
- **RULE-11 — related words together** (medium): keep subject next to verb; move a long parenthetical to the sentence's end or a new sentence.
- **RULE-12 — stress position** (medium): put the fact you want remembered at the sentence's end, not buried mid-sentence.
...
- **RULE-B — changelog narration** (critical): don't narrate past bugs, what the code used to do, or ticket numbers in code comments or docs. State current behavior, not what changed.
...
- **RULE-F — dash punctuation** (medium): no em/en dash as casual punctuation — use a comma, semicolon, colon, or parentheses instead. (Numeric ranges and paired names are fine.)
