...
collide. Two agents can never share `pr_branch`, since git refuses to check out one branch
...
3. Otherwise dispatch a fix agent (briefed to fix directly when every finding is `is_trivial()`,
   or to run `root-cause-fix` first when not, per Cross-references), append the fix-round summary,
...
to resume at round 2, which it was already past; with a cap of three, that guess spends the
...
checking a review agent's report before sorting its findings into the tracker; its §11 carries
...
coordinate_aspect(aspect, aspect_base, run_dir, efficacy_log):
    ...
    for pr IN aspect.prs:                     # ---- OUTER: one iteration per PR ----
        ...
        loop:                                 # ---- INNER: review rounds, at most three ----
            ...
            append_fix_summary(review_file, round)         # the fix-round summary: only what commits will not show
